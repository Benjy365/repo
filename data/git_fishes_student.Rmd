...
[My GitHub Repository](https://github.com/Benjy365/test)
...
## GitHub is just a storage, collaboration site in the cloud. There may be a great version backup feature. But the best storage is local, where you'll be able to open it more easily, even without the Internet.
...
##The rest of the data will be printed and echo will not be printed as an error.
```
...
new_data <- subset(data,select=-c(loc,comment))
# Check spelling in species and treatment but also generate a summary table
sorted_word<-names(sort(table(strsplit(tolower(paste(readLines("http://www.norvig.com/big.txt")))))))
correct<-function(word){c(sorted_words[adist(word,species)<=min(adsit(word,species))])}
